...
def user(username):
    ...
    return render_template('main.user.html', user=user, posts=posts.items, prev_url=prev_url, next_url=next_url)
...
def edit_profile():
    ...
    return render_template('main.edit_profile.html', title="Edit Profile", user=user, form=form)
...
def unfollow(username):
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash(_('User %{username} is not found:c ', username=username))
        return redirect(url_for('main.index'))
    if user == current_user:
